    ...
    function output_txt=PickCurveOnGraph(src,~,~)
        ...
        if isfield(parentfigure.UserData,'TrsSet')
           ...
           for iw = 2:numel(MFH.UserData.Wavelengths)+1
            RelCounts(iw) = (sum(Data(Ypos,Xpos,SETT.Roi(iw-1,2)+1:SETT.Roi(iw-1,3)+1),3)-mean(Data(Ypos,Xpos,str2double(MFH.UserData.BkgFirst.String):str2double(MFH.UserData.BkgLast.String)))*(numel(SETT.Roi(iw-1,2)+1:SETT.Roi(iw-1,3)+1)))./Counts * 100;         
