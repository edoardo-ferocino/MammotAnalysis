...
if isempty(allmh)
    if ~strcmpi(mfigobj.Tag,mfigobj.MainPanelTag)
        ...
        uimenu(ftmh,'Text','&backslash','Tag',[ftmh.Tag mfigobj.spacer 'backslash']);
        %% Separator
...
for is = 1:mfigobj.nSubMenu
    mfigobj.SubMenu(is).UserData.OriginalText = mfigobj.SubMenu(is).Text;
end
end